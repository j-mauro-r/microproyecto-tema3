...
def load_everything():
    ...
    lr_m.fit(X_tr_s, tr[TARGET].astype(int))
    ...
    def best_threshold(y_v, p_v):
        f1s = [f1_score(y_v, (p_v >= t).astype(int), zero_division=0) for t in thrs]
        return float(thrs[int(np.argmax(f1s))])

    p_val_xgb  = xgb_m.predict_proba(X_val)[:, 1]
    p_val_lgb  = lgb_m.predict_proba(X_val)[:, 1]
    p_val_lr   = lr_m.predict_proba(scaler.transform(X_val))[:, 1]
    p_val_gam  = gam_m.predict_proba(val[gam_feats].fillna(0).values)

    probs = {
        "XGBoost":   (xgb_m.predict_proba(X_te)[:, 1],               best_threshold(y_val, p_val_xgb)),
        "LightGBM":  (lgb_m.predict_proba(X_te)[:, 1],               best_threshold(y_val, p_val_lgb)),
        "Logistica": (lr_m.predict_proba(scaler.transform(X_te))[:,1], best_threshold(y_val, p_val_lr)),
        "GAM":       (gam_m.predict_proba(test[gam_feats].fillna(0).values), best_threshold(y_val, p_val_gam)),
    }
...
def build_cover(doc):
    ...
    for text, size, bold, italic in [
        ...
        ("Proyecto de Desarrollo de Soluciones — MAIA",
                                                               Pt(12), False, False),
        ("",                                                    Pt(11), False, False),
    ...
    for name in ["Sergio Ballesteros", "Stevan Ramirez", "Nicolas Lara", "Mauricio Rodriguez"]:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r = p.add_run(name)
...
def build_s2(doc, meta, results):
    ...
    add_body(doc,
        ...
        "etapa (build_features.py) calcula 28 predictoras organizadas en cuatro grupos. "
        "El primero son lags de 1, 2, 3, 4 y 6 meses y promedios móviles de tres meses para "
        "casos_clasico y casos_grave. El segundo son variables climáticas ERA5+CHIRPS (temperatura, "
        "precipitación, rocío, radiación solar, humedad del suelo) con cobertura del 96.3%, "
        "incluyendo lags de 1 a 3 meses para temperatura y precipitación. El tercero son "
        "componentes trigonométricas de estacionalidad (mes_sin, mes_cos). El cuarto son "
        "variables del canal endémico (p25, p75, zona_canal, zona_objetivo, sir, es_endemico, "
        "brote, brote_lag_1). La variable objetivo es la función de predicción objetivo, definida "
        "como si habrá brote (casos_clasico > p75) en el mes t+1."
    )
    add_body(doc,
    ...
    add_body(doc, text=(
        ...
        bold_label="GAM.")
    add_body(doc,
        "Se entrenó adicionalmente un modelo XGBoost T+2 (horizonte de dos meses) con "
        "AUROC=0.871, AP=0.840 y F1=0.758, lo que representa una degradación esperada "
        "respecto a T+1 dado el mayor horizonte de predicción. Ambos modelos T+1 y T+2 "
        "incluyen calibración isotónica post-hoc (IsotonicRegression sobre el conjunto de "
        "validación 2022-2023) que reduce el error de calibración Brier de XGBoost en un "
        "24% (0.1248→0.0947) y el de LightGBM en un 17% (0.1154→0.0954), habilitando el "
        "reporte de probabilidades estadísticamente válidas en el tablero de predicciones "
        "(campo probability del contrato de API v1.1.0).",
        bold_label="T+2 y calibración.")
    ...
    add_figure(doc, "report_inicios_brote.png",
               "Figura 4. Porcentaje de meses de inicio de brote (es_inicio=1) detectados "
               "por XGBoost y LightGBM (modelos de mayor desempeño) frente al canal endémico "
               "y la persistencia (métodos de referencia). Conjunto de prueba 2024-2025.",
               width=5.5)
...
def build_s3(doc):
    ...
    add_body(doc,
        "La arquitectura del sistema BIOMAC separa la capa de modelos de la capa de "
        "presentación mediante un contrato de API versionado (v1.1.0). El backend expone "
        "un endpoint FastAPI POST /api/v1/predictions que acepta municipios, mes de "
        "referencia y horizontes T+1/T+2, ejecuta el pipeline completo de features y "
        "devuelve una respuesta JSON estructurada con label (EXCESO/NO_EXCESO), "
        "probabilidad calibrada, regla de decisión, métricas de evaluación local y "
        "explicación SHAP. El frontend solo consume esta respuesta sin reconstruir ninguna "
        "lógica epidemiológica."
    )
    add_body(doc,
        "Las salidas del modelo cubren los tres campos que el contrato de API v1.1.0 "
        "declara opcionales pero deseables: (1) probabilidades calibradas — la calibración "
        "isotónica reduce el error Brier de XGBoost un 24%, permitiendo presentar en el "
        "tablero '62% de probabilidad de brote' con validez estadística; "
        "(2) prediction_history — se persiste un registro histórico de 92 inferencias "
        "(T+1 y T+2, Bucaramanga y Cali) con periodo, horizonte, probabilidad calibrada, "
        "umbral, label predicho y label observado, habilitando auditoría de 'pronosticado "
        "vs. ocurrido'; (3) explicación SHAP local — se calcularon valores SHAP mediante "
        "TreeExplainer de XGBoost sobre el conjunto de prueba filtrado por ciudad, "
        "permitiendo exponer explanation.method='shap', scope='local' con top_features "
        "por inferencia concreta. El feature de mayor contribución local en ambas ciudades "
        "es SIR (Susceptibles-Infectados-Recuperados estimado)."
    )
...
def build_s4(doc, meta, results):
    ...
    add_body(doc,
        ...
        "sino de información: las 39 features disponibles, incluyendo datos climáticos ERA5+CHIRPS "
        "con 96.3% de cobertura, el estado de brote actual y sus lags, y las variables del canal "
        "endémico, capturan la mayor parte de la señal predictiva accesible."
    )
    ...
    add_body(doc,
        f"El modelo T+2 (horizonte de dos meses) obtiene AUROC=0.871, AP=0.840, F1=0.758, "
        "con Bucaramanga F1=0.909 y Cali F1=0.909, confirmando la viabilidad de la "
        "anticipación de dos meses con degradación aceptable respecto a T+1."
    )
    add_body(doc,
        "Las principales limitaciones son la granularidad mensual, que impide capturar "
        "dinámicas intra-mensuales relevantes para la activación de respuestas inmediatas, "
        "y la cobertura piloto limitada a dos municipios. La incorporación de variables "
        "climáticas ERA5+CHIRPS al 96.3% de cobertura municipal no mostró ganancia "
        "sustancial en AUROC respecto a los modelos sin clima; la lluvia (cuatro rezagos) "
        "tiene coeficientes inferiores a 0.06 mientras que el punto de rocío es la tercera "
        "variable más importante. El período de prueba 2024-2025 presentó una tasa de "
        "brote del 43%, notablemente superior al histórico (14-23%), lo que indica un año "
        "epidémico severo en Colombia y puede sesgar al alza las métricas de prueba frente "
        "a años más típicos."
    )
