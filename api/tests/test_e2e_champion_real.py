...
CONTROLLED_RESULT = json.loads(CHAMPION_OUTPUT_PATH.read_text(encoding="utf-8"))
REFERENCE_MONTH = CONTROLLED_RESULT["reference_month"]
REFERENCE_YEAR, REFERENCE_MONTH_NUMBER = map(int, REFERENCE_MONTH.split("-"))
INVALID_MONTH_YEAR = REFERENCE_YEAR + 1 if REFERENCE_MONTH_NUMBER == 12 else REFERENCE_YEAR
INVALID_MONTH_NUMBER = 1 if REFERENCE_MONTH_NUMBER == 12 else REFERENCE_MONTH_NUMBER + 1
...
def test_full_local_http_sqlite_restart_and_read_only_flow(tmp_path):
    ...
    content = csv_bytes(year=REFERENCE_YEAR, month=REFERENCE_MONTH_NUMBER)
...
@pytest.mark.parametrize("content", [
    csv_bytes(year=REFERENCE_YEAR, month=REFERENCE_MONTH_NUMBER, omit=CHAMPION_FEATURES[0]),
    csv_bytes(year=REFERENCE_YEAR, month=REFERENCE_MONTH_NUMBER, municipalities=("68001",)),
    csv_bytes(year=REFERENCE_YEAR, month=REFERENCE_MONTH_NUMBER, extra_column="objetivo"),
    csv_bytes(year=INVALID_MONTH_YEAR, month=INVALID_MONTH_NUMBER),
    csv_bytes(year=REFERENCE_YEAR, month=REFERENCE_MONTH_NUMBER).replace(b"1.25", b"NaN", 1),
])
def test_invalid_uploads_stop_before_champion_and_preserve_latest(tmp_path, content):
    ...
    with TestClient(app, raise_server_exceptions=False) as client:
        good_run = _post(
            client, csv_bytes(year=REFERENCE_YEAR, month=REFERENCE_MONTH_NUMBER)
        ).json()["run"]["run_id"]
...
def test_unavailable_champion_is_sanitized_and_preserves_previous_latest(tmp_path):
    ...
    with TestClient(bad_app, raise_server_exceptions=False) as client:
        failed = _post(
            client,
            csv_bytes(year=REFERENCE_YEAR, month=REFERENCE_MONTH_NUMBER).replace(
                b"1.25", b"1.26", 1
            ),
        )
        latest = client.get("/api/v2/predictions/latest")
